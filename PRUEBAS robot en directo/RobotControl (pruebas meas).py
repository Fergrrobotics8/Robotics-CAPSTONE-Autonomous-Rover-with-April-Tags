...
class RobotControl(object):
    ...
    def process_measurements(self):
        ...
        """
        meas = self.ros_interface.get_measurements() #type list (matriz de 1x3 meas[0][2])
        ...
        if meas!=None:
            ...
            """
            if measu[1]>0.03:
                self.ros_interface.command_velocity(0.15, 0.5)  # el giro hacia la izquierda lo hace mas lento
                print("El tag ", measu[3], " esta en el lado positivo")
                print("Girando hacia la izquierda (1.3)")
            if measu[1]<-0.03:
                self.ros_interface.command_velocity(0.15, -0.5)
                print("El tag ", measu[3], " esta en el lado negativo")
                print("Girando hacia la derecha (-0.7)")
            if measu[1]<0.03 and measu[1]>-0.03:
                self.ros_interface.command_velocity(0.1, 0)
                print("Circulando recto")
